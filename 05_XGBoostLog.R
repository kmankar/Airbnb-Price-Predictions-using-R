...
#   Group#: 6
#   Script: Extreme Gradient Boosting (Logged)
#
###############################################
...
df$accommodates <- log(df$accommodates)
...
#
###############################
plotdata <- data.frame(exp(test.label),exp(xgbpred))
ggplot(data = plotdata, aes(x = exp.test.label., y = exp.xgbpred.)) + geom_point() + theme_tufte() + geom_abline()

###############################
#     KPI METRICS
#
###############################
test.label.mean <- exp(mean(test.label))
tss <- sum((exp(test.label) - test.label.mean)^2 )
rss <- sum(error^2)
rsq <- 1 - (rss/tss)
modelkpi[4,5] <- rsq
cal_metrics(4, exp(test.label), exp(xgbpred))
